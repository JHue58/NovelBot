...
class Client():
    ...
    def __init__(self,datajson={}):
        if datajson=={}:
            ...
            host = datajson['host']
            if host.find(':') == -1:
                datajson['host'] = datajson['host'] + ':' + datajson['port']
            try:
                host.strip('：', ':')
            except:
                pass
            self.data=datajson
        self.data=self.Get_Session()
    ...
    #激活会话ID（不要单独调用）
    def Check_Session(self,host, session, qq):
        url = 'http://' + host + '/bind'
    ...
    def Get_Session(self, getsession=0):
        host = self.data['host']
        verifyKey = self.data['Key']
        qq = self.data['qq']
        ...
        try:
            ...
            res = json.loads(res.text)
            session = res['session']
            code = self.Check_Session(host, session, qq)
            if (getsession == 0 and code == 0):
                self.data.update(session=session)
                return self.data
    ...
    def Fetch_Message(self, deal=1):
        ...
        Message = res['data']
        if deal == 1:
            Message = self.Fetch_Message_info(Message)
            return Message
        elif deal == 0:
            return Message
        else:
    ...
    #接受消息/事件信息(简化处理)
    def Fetch_Message_info(self,Message):
        messagemain = dict()
        messageinfo = dict()
        Message_c = []
        for i in Message:
    ...
    #返回值为本次发送的消息id
    def Send_Message(self, targets, target_type, message, message_type, path=0):
    ...
    #返回值为本次发送的消息id
    def Send_Message_Chain(self, targets, target_type, messagechain):
        targetlist=[]
        if type(targets)==type([]):
            targetlist.extend(targets)
        else:
    ...
    def Creat_data(self,host='127.0.0.1', port='8080', verifyKey='', qq='', session=''):
        host = str(host)
        port = str(port)
    ...
    def Recall_Message(self, messageid):
        session = self.data['session']
        host = self.data['host']
    ...
    #需要有相关权限
    def Unmute(self, target, member='0'):
        session = self.data['session']
        host = self.data['host']
        if member != '0':
    ...
    #需要有相关权限
    def Group_Config(self, target, **config):
        session = self.data['session']
        host = self.data['host']
    ...
    #需要有相关权限
    def Member_Info(self, target, memberid, **config):
        session = self.data['session']
        host = self.data['host']
    ...
    def Get_Groupmember(self, target, deal=1):
    ...
    def Get_memberinfo(self, target, id, deal=1):
        session = self.data['session']
        host = self.data['host']
        target = str(target)
        id = str(id)
